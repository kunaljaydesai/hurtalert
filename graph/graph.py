	...
	def shortest_path(self, start_pt, end_pt):
		...
		def get_shortest(start_node, end_node):
			if start_node is not None and end_node is not None:
				rt_node = RouteNode(start_node, start_node.get_distance_node(end_node), None)
				heap = [rt_node]
				visited = set()
				...
				while len(heap) != 0:
					...
					for node in connected:
						if node not in visited:
							visited.add(node)
							rt_node = RouteNode(node, node.get_distance_node(end_node), rt_start_node)
...
class RouteNode:
	...
	def __init__(self, node, distance, parent):
		...
		if parent is not None:
			self.cost = parent.cost + Cost(distance=node.get_distance_node(parent.node)).value()
...
#node_4.connect(node_1)
node_7.connect(node_4)
node_1.connect(node_7)
node_1.connect(node_2)
node_2.connect(node_5)
node_5.connect(node_4)


print([str(rt_node.node.point) for rt_node in g.shortest_path(Point(0, 0), Point(5, 5))])
